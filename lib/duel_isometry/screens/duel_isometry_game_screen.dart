...
import '../services/duel_isometry_validator.dart';
...
class _DuelGameScreenState extends ConsumerState<DuelGameScreen> {
  ...
  Future<void> _initializeAndLoadSolution() async {
    DuelIsometryValidator.instance.initialize(solutionMatcher.allSolutions);
    await _loadSolution();
  }

  Future<void> _loadSolution() async {
    final duelState = ref.read(duelIsometryProvider);
    final solutionId = duelState.solutionId;
    if (solutionId == null) return;

    final success = await DuelIsometryValidator.instance.loadSolution(solutionId);
    if (success) {
      _solutionGrid = DuelIsometryValidator.instance.solutionGrid;
    }
    setState(() {
  ...
  bool _canPlacePieceAt(Pento piece, int positionIndex, int gridX, int gridY, DuelIsometryState duelState) {
  ...
  _SnapResult? _findNearestValidPosition(Pento piece, int positionIndex, int anchorX, int anchorY, DuelIsometryState duelState) {
    _SnapResult? best;
  ...
  void _updatePreviewWithSnap(int rawX, int rawY, DuelIsometryState duelState) {
  ...
  Widget build(BuildContext context) {
    ...
    if (duelState.solutionId != null &&
        duelState.solutionId != DuelIsometryValidator.instance.currentSolutionId) {
      _loadSolution();
    }
    ...
    ref.listen<DuelIsometryState>(duelIsometryProvider, (previous, next) {
      ...
      if (next.gameState == DuelGameState.ended &&
        ...
        Navigator.pushReplacement(
          ...
          MaterialPageRoute(builder: (context) => const DuelIsometryWaitingScreen()),
        );
      }
    });
  ...
  Widget _buildScoreTitle(DuelIsometryState duelState) {
  ...
  Widget _buildGameBoard(BuildContext context, WidgetRef ref, DuelIsometryState duelState, settings) {
  ...
  bool _isPieceAtCell(DuelIsometryPlacedPiece placedPiece, int x, int y) {
  ...
  void _tryPlacePieceAt(WidgetRef ref, DuelIsometryState duelState, int pieceId) {
  ...
  void _tryPlacePiece(WidgetRef ref, DuelIsometryState duelState, int x, int y) {
    ...
    final validation = DuelIsometryValidator.instance.validatePlacement(
  ...
  Widget _buildPieceSlider(BuildContext context, WidgetRef ref, DuelIsometryState duelState, settings) {
  ...
  Widget _buildDraggablePiece(Pento piece, DuelIsometryState duelState, settings) {
