...
|---|---|---|
| **Défi de la semaine + classement en ligne** | `CAHIER_DES_CHARGES_V1.md` §7 | **HORS V1** (Paul, §12 Q3). **Phases 0-3 faites** : PRNG (0), dérivation `challenge.dart` (1), mode défi jouable local (2), **identité 128 bits** `AppSettings.playerId` + `generatePlayerId`/`ensurePlayerId` (3). **Phase 4 DÉPLOYÉE** (worker en ligne `https://pentapol-defi.pentapml.workers.dev`, D1 + `SEED_TOKEN` posés, round-trip validé au curl : POST 201, essai unique 409, leaderboard trié). **Client** : `challenge_api.dart` (POST score / GET tableau, échec silencieux §7.8) + **soumission auto à la complétion d'un défi** (`_submitChallengeScore`, `_activeChallenge`). **Phase 5 faite** : `LeaderboardScreen` (4 onglets jaune/à pois/vert/blanc, joueur courant surligné, dégradation gracieuse), accès par une icône « classement » sur `ChallengeScreen`. **Le défi est fonctionnel de bout en bout** (jouer → POST score → classement). **Reste optionnel** : fetch `GET /challenge` (composition à la main côté client, repli dérivation) + script Dart de semage |
