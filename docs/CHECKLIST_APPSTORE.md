...
| 19 | **Figer la règle des coups avant tout record publié** | CDC §12 **Q6** (Paul, 2026-09-03) : le déplacement direct d'une pièce posée ne compte **pas** comme un coup — le décompte doit **ignorer `translationCount`** et sommer poses + retraits. La formule est libre tant que rien n'est publié ; **une fois des records sur l'App Store**, la changer rend les anciens incomparables (règle n°6 de `CLAUDE.md`). À verrouiller **avant** la persistance des records (point 11). Le compteur de poses n'est peut-être pas encore matérialisé | `PentoscopeState` (compteurs), `PLAN_PERSISTANCE.md` §4, `CAHIER_DES_CHARGES_V1.md` §4.7 |
...
| # | Point | Note |
|---|---|---|
| 12 | **App Privacy** | À remplir en fonction de ce qui est réellement collecté. Après le retrait de Supabase et sans rapport de crash tiers, l'app ne collecte **rien** — c'est la déclaration la plus simple qui soit, et un argument à ne pas gâcher. Un outil de crash change cette réponse : le choisir en connaissance de cause. **Le classement en ligne — qui enverrait une identité persistante — étant hors V1 (CDC §12, Q3)**, la V1 conserve « ne collecte rien » (hors multijoueur, point 14) ; la déclaration changera à la mise à jour « classement » |
