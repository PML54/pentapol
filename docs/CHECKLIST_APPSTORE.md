...
| 21 | **Changer le bundle identifier** | `PRODUCT_BUNDLE_IDENTIFIER = com.example.pentapol` (iOS) et `applicationId = "com.example.pentapol"` (Android). `com.example.*` est un placeholder **refusé** à la soumission. À remplacer par un identifiant réel (ex. `com.pentapml.pentapol`) et enregistrer l'App ID correspondant. Signalé comme « nouveau bloquant » dans `JOURNAL.md`, absent de cette liste jusqu'au 2026-09-07 (`INDEX_DOCS.md` §3.2) | `ios/Runner.xcodeproj/project.pbxproj`, `android/app/build.gradle`, `FICHE_APP_STORE.md` |
| 22 | **Désactiver le bandeau de debug des compteurs** | `kShowLiveCounters = true` affiche un bandeau coin haut-gauche avec les compteurs live iso/fautes (aide au test device, 2026-09-07). **À repasser `false`** avant toute soumission — sinon un overlay de debug part en production | `lib/pentoscope/screens/pentoscope_game_screen.dart` |
